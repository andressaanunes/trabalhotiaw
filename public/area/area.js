...
function getProds() {
    ...
    fetch(`https://www."".com/area/${params[1]}`).then((res)=>{
        console.log(res)
        return res.json()
        ...
        prods.forEach(prod => {
            ...
            <div id= "columns" class="card h-100">
                ...
                <div class="card-body">
                    <h4 class="card-title"><a class="text-decoration-none" href="https://www."".com/produto?product=${prod.id}" title="View Product">${prod.nome}</a></h4>
                    <div class="align-text-bottom">
                        <span class="price"><strong>R$${prod.preco}</strong></span>
                    </div>
                </div>
                <div class="card-footer">
                    <a class="button" href="https://www."".com/produto?product=${prod.id}">Adicionar ao carrinho
                    </a>
                </div> 
            </div>
        </div>`
...
function searchProds() {
    ...
    const prods = fetch(`https://www."".com/search/${busca}`).then((res)=>{    
        console.log(prods)
        console.log(res)
